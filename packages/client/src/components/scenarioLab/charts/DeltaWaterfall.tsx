...
export default function DeltaWaterfall({ delta }: DeltaWaterfallProps) {
  ...
  const activeSteps = useMemo((): DeltaStep[] => {
    ...
    const filtered = rawSteps.filter(s => Math.abs(s.y) >= 1);
  ...
  const pointRender = useCallback((args: IPointRenderEventArgs): void => {
    ...
    if (!step) return;
    if (step.isNet) {
      args.fill = step.y >= 0 ? chartPalette.emerald : chartPalette.amber;
    } else {
      args.fill = step.y > 0 ? `${chartPalette.amber}99` : `${chartPalette.emerald}99`;
    }
  }, [activeSteps]);
