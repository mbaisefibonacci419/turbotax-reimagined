...
import { FileText, ChevronRight, ChevronLeft, Download, Printer, Loader2, CheckSquare, Square, Search, X, Sparkles, AlertTriangle, Check } from 'lucide-react';
...
export default function FormSidebar() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (selCount > 0) {
        ...
        filename = 'Nimbus_Selected_Forms.pdf';
      } else {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      if (selCount > 0) {
        pdfBytes = await generateSelectedFormsPDF(getSelections(), taxReturn, calculation);
        fallbackFilename = 'Nimbus_Selected_Forms.pdf';
      } else {
  ...
  return (
    <nav className="p-4" aria-label="Form navigation" style={{ color: 'var(--color-text-primary)' }}>
      {/* Switch back to the guided interview */}
      <button
        onClick={() => setViewMode('wizard')}
        className="w-full flex items-center gap-1.5 mb-3 rounded-md transition-colors"
        style={{
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--color-text-secondary)' }}>Forms</h2>
        {taxReturn && calculation && (
      ...
      {groupOrder.map(groupName => {
        ...
        return (
          <div key={groupName} className="mb-4">
            <h3 className="text-[11px] font-medium uppercase tracking-wider mb-1.5 px-2" style={{ color: 'var(--color-text-secondary)' }}>
              {groupName}
            </h3>
                      ...
                      <div
                        className={`w-full flex items-center gap-1.5 px-2 py-1.5 rounded-md text-sm transition-colors ${
                          isActive
                            ...
                            : isChecked
                              ? 'bg-telos-blue-600/10 text-slate-100'
                              : 'text-slate-100 hover:bg-surface-700'
                        }`}
                      >
                        <button
                          onClick={(e) => { e.stopPropagation(); toggleFormSelection(key); }}
                          className="shrink-0 text-slate-400 hover:text-telos-blue-400 transition-colors"
