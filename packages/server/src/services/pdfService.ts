...
/**
 * Sanitize a string for pdf-lib's WinAnsi encoding.
 * Replaces non-WinAnsi characters with ASCII approximations or '?'.
 * Standard PDF fonts (Helvetica, etc.) only support WinAnsi (Latin-1 superset).
 */
...
function drawRow(
  ...
  const valWidth = font.widthOfTextAtSize(safeValue, 9);
...
export async function generateForm1040PDF(
  ...
  page.drawText(`Tax Year ${taxReturn.taxYear}  |Prepared by Nimbus`, {
    x: MARGIN_L,
    y: cursor.y,
    size: 8,
    font: italic,
  ...
  if (taxReturn.ipPin) {
    drawRow(page, fonts, cursor, 'Identity Protection PIN', taxReturn.ipPin);
  }
  ...
  if (fs === FilingStatus.MarriedFilingJointly && taxReturn.spouseFirstName) {
    ...
    if (taxReturn.spouseIpPin) {
      drawRow(page, fonts, cursor, 'Spouse IP PIN', taxReturn.spouseIpPin);
    }
  }
  ...
  cursor.y -= 10;
  page.drawText(
    'Consult a qualified tax professional before filing. Generated by Nimbus.',
    { x: MARGIN_L, y: cursor.y, size: 7, font: italic, color: LIGHT_GRAY },
  );
...
export async function generateScheduleCPDF(
  ...
  const necTotal = taxReturn.income1099NEC.reduce((s, i) => s + i.amount, 0);
  const kTotal = taxReturn.income1099K.reduce((s, i) => s + i.grossAmount, 0);

  if (necTotal > 0) {
    drawRow(page, fonts, cursor, '1099-NEC income', fmtDollar(necTotal), { lineNum: '1' });
  }
  if (kTotal > 0) {
    drawRow(page, fonts, cursor, '1099-K income', fmtDollar(kTotal), { lineNum: '1' });
  }
  ...
  for (const [line, amount] of Object.entries(sc.lineItems).sort((a, b) => Number(a[0]) - Number(b[0]))) {
    const lineNum = Number(line);
    const label = LINE_LABELS[lineNum] || `Line ${lineNum}`;
    page = ensureSpace(cursor, 18, doc, fonts);
    drawRow(page, fonts, cursor, label, fmtDollar(amount), { lineNum: String(lineNum) });
  }
  ...
  if (sc.vehicleDeduction > 0) {
    drawRow(page, fonts, cursor, 'Vehicle deduction', fmtDollar(sc.vehicleDeduction));
  }
